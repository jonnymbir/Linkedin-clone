  ...
  return (
    ...
    >
      ...
      <div className="flex justify-evenly items-center dark:border-t border-gray-600/80 mx-2.5 pt-2 text-black/60 dark:text-white/75">
        {modalPost ? (
          <button className="postButton  flex space-x-2">
            <CommentOutlinedIcon />
            <h4>Comment</h4>
          </button>
        ) : (
          <button
            className={`postButton ${liked && "text-blue-500"}  flex space-x-2`}
        ...
        {session?.user?.email === post.email ? (
          <button
            className="postButton focus:text-red-400 flex space-x-2"
            onClick={deletePost}
          >
            <DeleteRoundedIcon />
            <h4>Delete post</h4>
